...
class MatchedStmts {
  public:
    // a new std::vector for each instance (aka match).
...
/// helper function to get reads and writes.
/// Restricts reads and write to node.
...
// FIXME: make me generic. ATM only works
// for 2-d arrays.
...
/// check: @map must be contained in the 
/// write accesses. 
...
// return reads and write that belong to node based on the restriction r.
std::pair<std::vector<isl::map>, std::vector<isl::map>> 
...
TEST(language, compilation) {
  ...
  auto m5 = loop(hasStmt(hasAccess(__anything())));
}
...
static lang::MatchedStmts groupStatements(
  const matchers::ScheduleNodeMatcher &m, const std::string &p, 
  lang::GroupStrategy g = lang::GroupStrategy::ONE_MATCH_ONE_INSTANCE) {
...
TEST(language, testTwentyThree) {
  ...
  #endif
  EXPECT_TRUE(res.matches_.size() == 2);
  EXPECT_TRUE(res.matches_[0].size() == 2);
